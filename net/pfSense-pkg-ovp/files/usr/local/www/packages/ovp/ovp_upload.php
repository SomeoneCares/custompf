...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
		...
		if (!empty($upload_errors)) {
			...
			if ($file_content === false || trim($file_content) === '') {
				...
				if (!empty($parsed['errors'])) {
					...
					$_SESSION['ovp_parsed']      = $parsed;
					$_SESSION['ovp_conflicts']   = $conflicts;
					$_SESSION['ovp_filename']    = htmlspecialchars($_FILES['ovpn_file']['name'], ENT_QUOTES, 'UTF-8');
					$_SESSION['ovp_description'] = trim(isset($_POST['description']) ? $_POST['description'] : '');
					$_SESSION['ovp_interface']   = trim(isset($_POST['interface'])   ? $_POST['interface']   : 'wan');
					$_SESSION['ovp_username']    = trim(isset($_POST['username'])    ? $_POST['username']    : '');
					$_SESSION['ovp_password']    = trim(isset($_POST['password'])    ? $_POST['password']    : '');
	...
	elseif ($_POST['action'] === 'import') {
		...
		if (empty($_SESSION['ovp_parsed'])) {
			$input_errors[] = "Session expired. Please upload the file again.";
		} else {
			$parsed    = $_SESSION['ovp_parsed'];
			$conflicts = isset($_SESSION['ovp_conflicts']) ? $_SESSION['ovp_conflicts'] : array();
			...
			$options = array(
				'description'      => $_SESSION['ovp_description'],
				'interface'        => $_SESSION['ovp_interface'],
				'username'         => $_SESSION['ovp_username'],
				'password'         => $_SESSION['ovp_password'],
				'import_ca'        => $import_ca,
			...
			unset(
				$_SESSION['ovp_parsed'],
				$_SESSION['ovp_conflicts'],
				$_SESSION['ovp_filename'],
				$_SESSION['ovp_description'],
				$_SESSION['ovp_interface'],
				$_SESSION['ovp_username'],
				$_SESSION['ovp_password']
			);
...
/* ----------------------------------------------------------------
   Restore preview state on page refresh
   ---------------------------------------------------------------- */
  ...
  <div id="main">
    <div class="container-fluid">
      ...
      <?php elseif ($step === 'preview' && $parsed !== null): ?>
        ...
        $prev_description = htmlspecialchars(isset($_SESSION['ovp_description']) ? $_SESSION['ovp_description'] : 'OVP Import', ENT_QUOTES, 'UTF-8');
        $prev_interface   = htmlspecialchars(isset($_SESSION['ovp_interface'])   ? $_SESSION['ovp_interface']   : 'wan',        ENT_QUOTES, 'UTF-8');
        $prev_filename    = htmlspecialchars(isset($_SESSION['ovp_filename'])    ? $_SESSION['ovp_filename']    : '',           ENT_QUOTES, 'UTF-8');
